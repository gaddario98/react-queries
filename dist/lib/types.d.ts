...
type MutateOptions<TData, TError, TVariables, TContext> = Parameters<ReturnType<typeof useMutation<TData, TError, TVariables, TContext>>["mutate"]>[1];
...
export type QueryDefinition<K extends string, T extends "query" | "mutation", P, R, C = any> = {
    key: K;
    props: P;
    response: R;
    converter: C;
...
export type ExtractQuery<Q extends QueriesArray> = Extract<Q[number], {
    type: "query";
}>;
export type ExtractQueryResponse<Q extends QueriesArray, K extends Q[number]["key"]> = Extract<Q[number], {
    key: K;
}>;
export type ExtractMutation<Q extends QueriesArray> = Extract<Q[number], {
    type: "mutation";
}>;
export type AllMutation<Q extends QueriesArray = QueriesArray> = {
    [K in ExtractMutation<Q>["key"]]: ReturnType<typeof useMutation<ExtractQueryResponse<Q, K>["response"], Error, ExtractQueryResponse<Q, K>["props"], unknown>>;
};
export interface MutationConfig<TProps = unknown, TResponse = unknown, TConverter = any> extends QueriesProps<TProps, TResponse, TConverter> {
    method: "POST" | "PUT" | "DELETE";
}
export type SingleQueryConfig<Q extends QueryDefinition<any, any, any, any>> = Q extends QueryDefinition<infer K, infer T, infer P, infer R, infer C> ? T extends "mutation" ? {
    type: "mutation";
...
export type QueryAtIndex<Q extends QueriesArray, I extends keyof Q> = Q[I] extends QueryDefinition<infer K, infer T, infer P, infer R, infer C> ? QueryDefinition<K, T, P, R, C> : never;
...
export type MutationItem<Q extends QueriesArray> = Extract<QueryConfigArray<Q>[number], {
    type: "mutation";
    mutationConfig: any;
}>;
export type QueryItem<Q extends QueriesArray> = Extract<QueryConfigArray<Q>[number], {
    type: "query";
}>;
export {};
